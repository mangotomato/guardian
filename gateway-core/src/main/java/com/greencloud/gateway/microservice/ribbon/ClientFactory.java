package com.greencloud.gateway.microservice.ribbon;
 ...
 * support dynamic config argument. <=leejianhao
 * @author awang,leejianhao
 *
 */
public class ClientFactory {
    ...
    public static synchronized IClient getNamedClient(String name, IClientConfig clientConfig) throws ClientException {
		IClient<?, ?> client = simpleClientMap.get(name);
		if (client != null) {
			return client;
		}
		client = registerClientFromProperties(name, clientConfig);
		simpleClientMap.put(name, client);
		return client;
	}
