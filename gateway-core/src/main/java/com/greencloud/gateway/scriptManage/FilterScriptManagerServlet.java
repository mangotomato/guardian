...
import com.greencloud.gateway.util.JSONUtil;
...
public class FilterScriptManagerServlet extends HttpServlet {
    ...
    public static final DynamicBooleanProperty adminEnabled =
            DynamicPropertyFactory.getInstance().getBooleanProperty(GatewayConstants.GATEWAY_FILTER_ADMIN_ENABLED, true);
    ...
    private IGatewayFilterDAO scriptDAO;
    ...
    public FilterScriptManagerServlet() {
        this(GatewayFilterDAOFactory.getGatewayFilterDAO());
     ...
     * @param scriptDAO
     */
    private FilterScriptManagerServlet(IGatewayFilterDAO scriptDAO) {
        super();
    ...
    private void handleListAction(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (filter_id == null) {
            ...
            response.getWriter().write(JSONUtil.jsonFromMap(json));
        } else {
            ...
            if (Boolean.parseBoolean(request.getParameter("active"))) {
                ...
                // get list of all scripts for this endpoint
                scripts = scriptDAO.getGatewayFilters(filter_id);
            }
            if (scripts.size() == 0) {
