...
public class RibbonFilter extends GatewayFilter {
    ...
    public Object run() throws GatewayException {
        ...
        try {
            ...
            RestClient client = (RestClient) com.greencloud.gateway.microservice.ribbon.ClientFactory.getNamedClient(serviceName, clientConfig);

            HttpResponse response = forward(client, clientConfig, verb, uri, serviceName, headers, requestEntity);
            setResponse(response);
            tran.setStatus(Transaction.SUCCESS);
        } catch (Exception ex) {
            tran.setStatus(ex);
            String errorMsg = String.format("Origin URL: %s => Target URL: %s error, cause: %s",
                    request.getRequestURL(), uri, ex.getMessage());
    ...
    public HttpResponse forward(AbstractLoadBalancerAwareClient client, IClientConfig clientConfig, String verb,
                                String uri, String serviceName, Collection<Header> headers, InputStream entity)
    ...
    private IClientConfig buildRequestConfig(String serviceName) {
        ...
        clientConfig.setProperty(CommonClientConfigKey.MaxAutoRetries, RibbonPropertyHelper.getRibbonMaxAutoRetries
                (serviceName));
